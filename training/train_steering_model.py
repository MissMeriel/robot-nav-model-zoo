...
def parse_args():
    ...
    print(args.path_to_trainingdir, args.noisevar)
    return args
...
def main_pytorch_model():
    ...
    newdir = f"{model._get_name()}-{NB_EPOCH}epoch-{timestr}-{randstr}"
    if not os.path.exists(newdir):
        os.mkdir(newdir)
        shutil.copy(f"{__file__}", newdir)
    iteration = f'{model._get_name()}-randomblurnoise-{input_shape[0]}x{input_shape[1]}-lr1e4-{NB_EPOCH}epoch-{BATCH_SIZE}batch-lossMSE-{int(dataset.get_total_samples()/1000)}Ksamples-INDUSTRIALandHIROCHIandUTAH-noiseflipblur'
        ...
        for i, hashmap in enumerate(trainloader, 0):
            ...
            if i % logfreq == logfreq-1:  # print every 2000 mini-batches
                ...
                if (running_loss / logfreq) < lowest_loss:
                    ...
                    model_name = f"./{newdir}/model-{iteration}-best.pt"
    ...
    for epoch in range(NB_EPOCH):
        os.remove(f"./{newdir}/model-{iteration}-epoch{epoch}.pt")
    print(f"Saving model to {model_name}")
    print("All done :)")
